...
namespace FindMe_Application
{
    public class PinPage : ContentPage
    {
        ...
        Location initialPosition = new Location(43.8971, -78.8658); //oshawa when open map
        public PinPage()
        {
            //new map object 
            map = new Map
        ...
        public async Task ProcessFirstSmsLine(string sms)
        {
                ...
                DateTime.TryParseExact(parts[3], "dd/MM/yyyy", null, System.Globalization.DateTimeStyles.None, out DateTime parsedDate))
            {
                ...
                {
                    Location = new Location(formattedLAT, formattedLONG),
                    Label = $"{parsedDate.ToString("dd/MM/yyyy")} {parsedTime.ToString("HH:mm")}"
                });

                map.MoveToRegion(MapSpan.FromCenterAndRadius(new Location(formattedLAT, formattedLONG), Distance.FromMiles(0.2)));
            }
            else
            {
        ...
        private async Task ProcessMapRegion(string line)
        {
           // await DisplayAlert("processsmap", line, "ok");
                ...
                double.TryParse(parts[2], out double regionLongitude))
            {
                ...
                // Set the map region
                map.MoveToRegion(MapSpan.FromCenterAndRadius(new Location(formattedLAT, formattedLONG), Distance.FromMiles(0.05)));
            }
            else
            {
